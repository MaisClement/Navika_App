...
class TraficDetails extends StatelessWidget {
  ...
  String getTime(String time) {
    ...
    String dthour =
        dttime.hour < 10 ? '0${dttime.hour}' : dttime.hour.toString();
    String dtminute =
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              ...
                              color: getSlug(
                                  getTraficLines(
                                      LINES.getLines(lineId).id)['severity'],
                                  1),
                            ),
                          ),
                          child: Image(
...
// CTRAFIC
              if (getTraficLines(LINES.getLines(lineId).id)['reports'] != null)
                for (var ctrafic
                    in getTraficLines(LINES.getLines(lineId).id)['reports']
                        ['current_trafic'])
                  Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
...
// FWORK
              if (getTraficLines(LINES.getLines(lineId).id)['reports'] !=
                      null &&
                  getTraficLines(LINES.getLines(lineId).id)['reports']
                              ['future_work']
                          .length >
                      0)
                Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        for (var ctrafic in getTraficLines(
                                LINES.getLines(lineId).id)['reports']
                            ['future_work'])
                          Container(
                            margin: const EdgeInsets.only(
                                top: 5, right: 5, bottom: 15.0),
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (ctrafic['message']['title'] != null)
